...
blueon
		LDR R0,=GPIOD_PCOR ;Load address of GPIOB_PCOR to R0 
		LDR R1,=BLUE_MASK ;Load value to R1 
		STR R1,[R0] ;Put value into GPIOB_PCOR 
		BX LR	

blueoff
		LDR R0,=GPIOD_PSOR ;Load address of GPIOB_PSOR to R0 
		LDR R1,=BLUE_MASK ;Load value to R1 
		STR R1,[R0] ;Put value into GPIOB_PSOR 
		BX LR
...
bluetoggle
		LDR R0,=GPIOD_PTOR ;Load address of GPIOD_PTOR to R0 
		LDR R1,=BLUE_MASK ;Load value to R1 
		STR R1,[R0] ;Put value into GPIOD_PTOR
		BX LR
...
init_gpio							; Turns on clocks for PORTB and PORTD 
		...
		ORRS R1,R2 					;OR original value with mask to set bits AND R1,R3 ;AND value with mask to clear bits 
