...
SIM_SCGC6   EQU 0x4004803C ; SIM_SCGC5 address
...
loop	
		LDR R0,=SIM_SCGC6 			;Load address of SIM_SCGC5 to R0 
		LDR R1,[R0] 				;Get original value of SIM_SCGC5 
		LDR R2,=0x08000000 			;Load mask for bits to set to R2 
		ORRS R1,R2 					;Set bits with OR of orig val and mask 
		STR R1,[R0] 
		

		
		LDR R0,=PORTE_PCR20 			;Load address of SIM_SCGC5 to R0 
		LDR R1,=0x00000000 			;Load new value to R1 (Enabling MUX for ;GPIO operation)
		STR R1,[R0] 				
		
		
		LDR R0,=SIM_SCGC5 			;Load address of SIM_SCGC5 to R0 
		LDR R1,[R0] 				;Get original value of SIM_SCGC5 
		LDR R2,=0x00002000 			;Load mask for bits to set to R2 
		ORRS R1,R2 					;Set bits with OR of orig val and mask 
		STR R1,[R0] 
